...
export function usePicoclaw() {
  const [status, setStatus] = useState<PicoclawStatusPayload>({
    status: "disconnected",
  })
  ...
  useEffect(() => {
    ...
    if (window.picoclaw) {
      ...
      offStatus = window.picoclaw.onStatus((payload) => {
        setStatus(payload)
        addLog({
          direction: "system",
      ...
      offMessage = window.picoclaw.onMessage((payload) => {
        addLog({
      ...
      addLog({
        direction: "system",
        data: "Picoclaw bridge unavailable",
        ts: Date.now(),
      })
    }
  ...
  const handleConnect = async () => {
    try {
      const payload = await window.picoclaw.connect()
      setStatus(payload)
    } catch {
      addLog({
        direction: "system",
  ...
  const handleDisconnect = async () => {
    try {
      const payload = await window.picoclaw.disconnect()
      setStatus(payload)
    } catch {
      addLog({
        direction: "system",
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await window.picoclaw.send(payload)
      if (!result.ok) {
        addLog({
          direction: "system",
